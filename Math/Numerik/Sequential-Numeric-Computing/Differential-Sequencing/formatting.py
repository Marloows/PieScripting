...
def fetch_header(n:int, var:tuple = ('t', 'y'), gen_dy_dt: Callable = lambda y: (lambda n: y + n*"'")):
   t, y = var
   dy_dt = gen_dy_dt(y)
...
def markdown_view(A:Iterable, verbose:bool = True, fetch_header:Callable = fetch_header) -> str:
...
def show(A:Callable, n: int = 10, rounded2:int = 6) -> str:

   rounded = lambda X: tuple(map(lambda x: f"{round(x, rounded2)}", X))   # row-wise

   S = tuple(map(rounded, loop(A(), n)))

   return table_view(S, verbose=True)
